...
class Database:
    def __init__(self):
        self.storage_file = Path(__file__).resolve().parent / "data_store.json"
        self.jobs: Dict[str, dict] = {}
        self.applications: Dict[str, dict] = {}
        self.candidates: Dict[str, dict] = {}
        self.load_from_disk()
        self.sanitize_loaded_jobs()
        self.prune_temporary_jobs()
        if self.is_empty():
            self.seed_data()
            self.save_to_disk()

    def is_empty(self) -> bool:
        return not self.jobs and not self.applications and not self.candidates
    ...
    def load_from_disk(self):
        ...
        try:
            ...
            self.applications = payload.get("applications", {})
    ...
    def sanitize_job_description(self, description):
        if not description:
            return ""
    ...
    def sanitize_loaded_jobs(self):
        for job in self.jobs.values():
            job["description"] = self.sanitize_job_description(job.get("description"))

    def prune_temporary_jobs(self):
        temporary_titles = {"Persisted Admin Role", "Job11 Demo"}
        temporary_ids = {
            "adbdebff-2633-4bc9-804c-3f033ac82dc0",
        }
        removed_any = False
        for job_id in list(self.jobs.keys()):
            job = self.jobs[job_id]
            if job.get("title") in temporary_titles or job.get("id") in temporary_ids:
